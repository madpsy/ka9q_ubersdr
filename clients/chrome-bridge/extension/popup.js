// UberSDR Bridge — Popup Script (Chrome MV3)
'use strict';

// ── browser shim ───────────────────────────────────────────────────────────────
// Firefox exposes a Promise-based `browser` API.
// Chrome exposes `chrome.*` which returns Promises in Chrome 99+ but older
// versions use callbacks. We build a minimal Promise-based shim so the popup
// can use `.catch()` safely on all Chrome versions.
const browser = (typeof globalThis.browser !== 'undefined') ? globalThis.browser : {
    ...
    tabs: {
        ...
        query:   (opts)        => chrome.tabs.query(opts),
        sendMessage: (id, msg) => new Promise((resolve, reject) => {
            try {
...
browser.runtime.onMessage.addListener((msg) => {
    ...
    switch (msg.type) {
        case 'registry:updated':
            ...
            if (typeof renderEnabledWidgets === 'function') renderEnabledWidgets();
            if (typeof renderCatalogueDropdown === 'function') renderCatalogueDropdown();
            if (widgetCatalogueSelect && widgetCatalogueSelect.value && typeof showWidgetPreview === 'function') showWidgetPreview(widgetCatalogueSelect.value);
            break;
...
function showWidgetPreview(widgetId) {
    if (!widgetId) { widgetPreview.style.display = 'none'; return; }
    const w = widgetCatalogue.find(x => x.widget_id === widgetId);
    if (!w) { widgetPreview.style.display = 'none'; return; }
    ...
    try {
        ...
        if (resp && resp.ok) {
            ...
            if (resp.widgetCache && typeof resp.widgetCache === 'object') {
                ...
                const ww = widgetCatalogue.find(x => x.widget_id === widgetId);
                if (ww) widgetCacheLocal[widgetId] = { name: ww.name, callsign: ww.callsign || '' };
            }
            renderEnabledWidgets();
            renderCatalogueDropdown();
            showWidgetPreview(widgetId);
...
// Handle widgets:updated and suppressed_widgets:updated broadcasts from background.
browser.runtime.onMessage.addListener((msg) => {
    if (!msg || !msg.type) return;
    if (msg.type === 'widgets:updated') {

...
t('flrig sync runs one way only', () => {
    // Two-way sync is a feedback loop held apart by a timer, and anything
    // arriving inside that window — a real move on the rig, a VFO switch — is
    // indistinguishable from the echo and gets dropped.
    const html = read(path.join(EXT, 'popup.html'));
    assert.ok(!/value="both"/.test(html), 'the "both ways" option is back in the popup');
    const src = code(background);
    // The migration reads `stored.flrigDirection`; what must not come back is
    // the live variable being treated as a third direction.
    assert.ok(!/(?<!stored\.)flrigDirection === 'both'/.test(src), 'background still honours "both"');
    // A stored 'both' from an older version has to land somewhere valid.
    assert.ok(/=== 'both' \? 'sdr-to-rig'/.test(src), 'no migration for a stored "both"');
});

t('the frequency box has no stepper of its own', () => {
    // A number input steps its own value with the arrows and the keyboard, and
    // 1 Hz nudges made that way did not survive the round trip while the same
    // value typed in did. The popup computes and validates the frequency
    // itself; nothing else should also be editing it.
    const html = read(path.join(EXT, 'popup.html'));
    const field = html.match(/<input[^>]*id="input-freq"[^>]*>/);
    assert.ok(field, 'the frequency input is gone');
    assert.ok(!/type="number"/.test(field[0]), 'the frequency box is a number input again');
    assert.ok(!/\bstep=/.test(field[0]), 'the frequency box has a step again');
});

t('nothing mutes the browser tab', () => {
    // Tab mute was chosen for "instant response" against v1, whose mute flushed
    // the audio queue; v2's is a 15 ms gain ramp. It is invisible to the page,
    // so the receiver went on showing itself unmuted with nothing coming out of
    // it — and the popup's button read the receiver's mute while writing the
    // tab's, which is how the icon came to disagree with what you could hear.
    for (const [name, src] of [['background', code(background)], ['popup', code(popup)]]) {
        assert.ok(!/tabs\.update\([^)]*muted/.test(src), `${name}.js still mutes the browser tab`);
    }
    // The popup drives the receiver's mute, and nothing else claims to.
    assert.ok(/cmd:set_mute/.test(code(popup)), 'the popup no longer mutes anything');
    assert.ok(!/popup:set_tab_mute/.test(code(popup) + code(background)),
        'the tab-mute message is back');
});

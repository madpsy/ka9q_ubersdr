...
browser.runtime.onMessage.addListener((msg) => {
    ...
    switch (msg.type) {
        ...
        case 'suppressed_widgets:updated':
            // Background sends { origin, suppressedWidgets } — extract the list for
            // the active tab's origin directly from the message (no async round-trip).
...
function renderEnabledWidgets() {
    ...
    // Suppressed widgets show a restore button; active ones show a hide button.
    ...
    function buildInstanceWidgetItem(w, isSuppressed) {
        ...
        if (isSuppressed) {
            // Restore button — navigates tab back without this exclusion.
            ...
            // Hide button — navigates tab to ?exclude_widgets=uuid so server omits it.

...
t('PTT mute is absolute, so a missed message cannot invert it', () => {
    const p = attached();               // starts unmuted
    p.fromBackground({ type: 'cmd:set_mute', muted: false });
    assert.deepStrictEqual(p.calls, [], 'already unmuted: nothing to do');
    p.fromBackground({ type: 'cmd:set_mute', muted: true });
    assert.deepStrictEqual(p.calls, [['toggleMute']]);
});

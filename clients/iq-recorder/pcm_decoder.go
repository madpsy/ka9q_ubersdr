...
func NewPCMBinaryDecoder() (*PCMBinaryDecoder, error) {
	...
	if err != nil {
		...
		return decoder, fmt.Errorf("failed to create zstd decoder: %w", err)
	}
...
//   - remaining: PCM data (big-endian int16)
//
// Returns: PCM data as bytes (int16, little-endian), sample rate, channels, GPS timestamp (nanoseconds), error
func (d *PCMBinaryDecoder) DecodePCMBinary(binaryData []byte, isZstd bool) ([]byte, int, int, uint64, error) {
	// Decompress entire packet first if zstd
	if isZstd {
		if !d.zstdAvailable || d.zstdDecoder == nil {
			return nil, 0, 0, 0, fmt.Errorf("received zstd-compressed PCM but zstd not available")
		}

		decompressed, err := d.zstdDecoder.DecodeAll(binaryData, nil)
		if err != nil {
			return nil, 0, 0, 0, fmt.Errorf("zstd decompression error: %w", err)
		}
		binaryData = decompressed
	}

	if len(binaryData) < 4 {
		return nil, 0, 0, 0, fmt.Errorf("binary PCM packet too short: %d bytes", len(binaryData))
	}
	...
	var gpsTimestampNanos uint64
	var pcmData []byte

	if magic == 0x5043 { // "PC" - Full header
		if len(binaryData) < 29 {
			return nil, 0, 0, 0, fmt.Errorf("full header PCM packet too short: %d bytes", len(binaryData))
		}

		// Parse full header (little-endian)
		// GPS timestamp is at offset 4-12 (8 bytes, nanoseconds since Unix epoch)
		gpsTimestampNanos = binary.LittleEndian.Uint64(binaryData[4:12])
		// Wall clock time at offset 12-20 is deprecated (contains GPS time in milliseconds)
		...
		if len(binaryData) < 13 {
			return nil, 0, 0, 0, fmt.Errorf("minimal header PCM packet too short: %d bytes", len(binaryData))
		}

		// Parse minimal header (little-endian)
		// GPS timestamp is at offset 3-11 (8 bytes, nanoseconds since Unix epoch)
		gpsTimestampNanos = binary.LittleEndian.Uint64(binaryData[3:11])
		...
		if sampleRate == 0 || channels == 0 {
			return nil, 0, 0, 0, fmt.Errorf("received minimal header before full header")
		}

	} else {
		return nil, 0, 0, 0, fmt.Errorf("invalid PCM magic bytes: 0x%04X", magic)
	}
	...
	for i := 0; i < numSamples; i++ {
		// Read big-endian int16 and write as little-endian int16
		// Must preserve the bit pattern exactly (signed values)
		sample := binary.BigEndian.Uint16(pcmData[i*2:])
		binary.LittleEndian.PutUint16(pcmLE[i*2:], sample)
	}

	return pcmLE, sampleRate, channels, gpsTimestampNanos, nil
}

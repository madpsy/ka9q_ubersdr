...
// pcmProtocolVersion is the audio protocol this recorder speaks, and the only
// one it reads.
//
...
// carrying only what changed. This recorder captures iq48, where the saving is
// about 30% -- against zstd's previous contribution of nothing at all: it is an
...
// than the samples it carried. Over a long unattended capture that difference
// is written to disk.
//
// The timestamp matters here more than it does elsewhere: multi-instance
// recordings are aligned against it (see the alignment path in receiveData),
// and version 4 carries the same GPS-synchronised nanoseconds versions 1-3 put
// in their fixed header -- absolute at each resynchronisation point, a varint
// delta in between.
//

...
// DecodePacketLE is DecodePacket in the shape the rest of this client works in:
// little-endian int16 bytes plus the stream parameters, ready to be written
// straight into the WAV body.
//
// Little-endian is what the file wants and what the codec already produces, so
// unlike the versions 1-3 path this involves no byte swapping: those carried
// big-endian samples that had to be reversed on arrival.
//
// The timestamp is returned because multi-instance captures are aligned on it;
// it is the same GPS-synchronised nanoseconds the older header carried.
//
// The returned slice is freshly allocated on every call because it is buffered
// and handed on.
func (d *PCMv4StreamDecoder) DecodePacketLE(pkt []byte) (pcmLE []byte, sampleRate, channels int, timestampNanos uint64, err error) {
	...
	return pcmLE, h.SampleRate, h.Channels, h.TimestampNanos, nil
}

// zstdMagic is the zstd frame magic, 0xFD2FB528 little-endian on the wire.
//
// This client speaks protocol version 4 only, which never wraps a packet in
...
// that into a message the operator can act on rather than an unattended
// recording that produces an empty file.

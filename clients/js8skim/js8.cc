                ...
                const ffts_t &m79)
{
  ...
  if(decode(ll174, a174, use_osd, comment)){
    ...
    if(cb_ != 0){
      // Extract i3 bits (transmission type) from bits 72-74 of the decoded message
      int i3 = (a174[87+72] << 2) | (a174[87+73] << 1) | a174[87+74];
      
      cb_mu_.lock();
      int ret = cb_(a174+87, best_hz + down_hz_, best_hz + down_hz_,
                    best_off, comment.c_str(), snr, pass_, correct_bits, i3);
      cb_mu_.unlock();
      if(ret == 2){

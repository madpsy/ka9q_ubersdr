...
// Stub - WAV support removed for RX-only operation
void
writewav(const std::vector<double> &samples, const char *filename, int rate)
{
  fprintf(stderr, "writewav() not supported in RX-only build\n");
}
...
{
  fprintf(stderr, "readwav() not supported in RX-only build\n");
  exit(1);
}

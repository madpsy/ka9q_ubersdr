...
class AddInstanceDialog:
    ...
    def _create_widgets(self):
        ...
        # Host
        ttk.Label(self.dialog, text="Host:").grid(row=1, column=0, sticky=tk.W, padx=10, pady=5)
        self.host_var = tk.StringVar(value="localhost")
        ttk.Entry(self.dialog, textvariable=self.host_var, width=30).grid(row=1, column=1, padx=10, pady=5)
        
        # Port
        ttk.Label(self.dialog, text="Port:").grid(row=2, column=0, sticky=tk.W, padx=10, pady=5)
        self.port_var = tk.StringVar(value="8080")
        ttk.Entry(self.dialog, textvariable=self.port_var, width=30).grid(row=2, column=1, padx=10, pady=5)
        
        # TLS
        self.tls_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(self.dialog, text="Use TLS", variable=self.tls_var).grid(row=3, column=1, 
                                                                                 sticky=tk.W, padx=10, pady=5)
        
        # Frequency
        ttk.Label(self.dialog, text="Frequency (MHz):").grid(row=4, column=0, sticky=tk.W, padx=10, pady=5)
        self.freq_var = tk.StringVar(value="14.100")
        ttk.Entry(self.dialog, textvariable=self.freq_var, width=30).grid(row=4, column=1, padx=10, pady=5)
...
class EditInstanceDialog:
    ...
    def __init__(self, parent, instance: SpectrumInstance, on_ok: Callable[[SpectrumInstance], None]):
        ...
        self.dialog = tk.Toplevel(parent)
        self.dialog.title(f"Edit {instance.name}")
        self.dialog.geometry("400x250")
        self.dialog.transient(parent)
        self.dialog.grab_set()

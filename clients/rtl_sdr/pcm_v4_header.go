...
// pcmProtocolVersion is the audio protocol this bridge speaks, and the only one
// it reads.
//
...
// carrying only what changed. On the rates this bridge uses, 384 kHz IQ falls
// from 1590 kB/s to 1116 and 48 kHz from 199.6 to 140.4 -- about 30%, against
// zstd's previous contribution of nothing at all: it is an LZ77 matcher over
// bytes, and a band-limited RF signal has no repeated byte strings, so every IQ
// mode measured at 0.99x, the compressed stream larger than the samples it
// carried.
//

...
// Uses a narrow 10 kHz spectrum window to detect and track the reference signal
...
type FrequencyReferenceMonitor struct {
	...
	// Narrow spectrum session (10 kHz wide)
...
func NewFrequencyReferenceMonitor(config *Config, radiod *RadiodController, sessions *SessionManager) (*FrequencyReferenceMonitor, error) {
	...
	frm := &FrequencyReferenceMonitor{
		...
		binCount:     1024,
		binBandwidth: 10000.0 / 1024.0, // 10 kHz / 1024 bins = ~9.77 Hz/bin
	}
...
func (frm *FrequencyReferenceMonitor) Start() error {
	...
	log.Printf("Creating frequency reference spectrum session at %.6f MHz (10 kHz wide, %.2f Hz resolution)",
	...
	// Create narrow spectrum channel (10 kHz wide centered on reference frequency)
	...
	frm.refSpectrum = &BandSpectrum{
		Band: NoiseFloorBand{
			...
			Start:           frm.centerFreq - 5000, // -5 kHz
			End:             frm.centerFreq + 5000, // +5 kHz
	...
	log.Printf("Frequency reference monitor started (%.6f MHz ± 5 kHz, %.2f Hz resolution)",
		float64(frm.centerFreq)/1e6, frm.binBandwidth)

...
- New *Native Desktop Clients* for the V2 interface. These clients exist for all major operating systems and architechtures including Windows, Mac and Linux. They can be found on the UberSDR.org website and within the links menu (top left ubersdr icon) in the UI
- Configabble *Bin sizes and update rate* which essentially means how detailed and how fast spectrum data is pushed to clients. You can configure these by re-running the setup wizard (base url/setup-wizard.html) on your instance - they are in Step 2 under 'Resolution' and 'Update Rate'. I still default to 'Medium' for both as there are CPU and Network Bandwidth tradeoffs, but feel free to experiment
...
- Added **QRSS Grabber** frontend extension (enable in Admin -> Extensions) for viewing very slow speed CW
...
- Added **Weather** based on instance location. Displayed in a few pages via a public endpoint (/api/weather)
...
- Telegram Bot support was added as part of the notification system. This allows you to send commands to the bot and recieve a response from your instance in real time. Examples are view current users, band conditions, system health and PSKr ranking, just to name a few. It can also optionally allow write commands such as change rotator position, antenna switch port and force a version upgrade of the system. It also has a Chat Relay meaning you can join the chat of your instance from Telegram and send/receive chat messages.
...
- CW Skimmer will NOT work on ARM64 - everything else appears to work (albeit some addons may be too much for a Pi 5)

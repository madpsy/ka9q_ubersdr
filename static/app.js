...
const wsManager = new WebSocketManager({
    ...
    onDisconnect: () => {
        ...
        stopStatsUpdates();
        // Reset the SNR squelch slider to Off on disconnect.
        // The last user-set value is preserved in _lastUserSNRSliderValue (updated
        // whenever the user moves the slider) and restored in onConnect.
...
// / notification media widget.
const _isMobileChrome = /Android/i.test(navigator.userAgent) &&
    /Chrome/i.test(navigator.userAgent) && !_isApple;

// ── MediaSession platform strategy ──────────────────────────────────────────
//
// Three paths depending on the platform:
//
// 1. _useMediaSessionBridge = true  (Apple Safari, Firefox)
//    Uses MediaStreamDestination → hidden <audio srcObject> bridge.
//    Audio routes through the bridge element; no HTTP stream needed.
//    These browsers require an <audio> element to be playing for
//    MediaSession to work.
//
// 2. _isMobileChrome = true  (Android Chrome)
//    Uses HTTP audio stream → hidden <audio src="/audio/stream">.
//    Android Chrome requires a URL-based <audio> element for the
//    lock-screen / notification media widget.
//
// 3. Neither (desktop Chrome/Edge) — "metadata-only"
//    Just sets navigator.mediaSession.metadata and playbackState.
...
function updateMediaSession() {
    ...
    //
    // Bridge path (Apple/Firefox) and desktop Chrome (metadata-only) have no
    // <audio src> element, no buffering phase — safe to set metadata immediately.
    // Only Android Chrome uses the HTTP stream path and must wait.
    if (_isMobileChrome && mediaSessionEnabled && !_httpStreamPlaying) {
    ...
    // For bridge path (Apple/Firefox) and desktop Chrome (metadata-only),
...
document.addEventListener('DOMContentLoaded', () => {
                ...
                navigator.mediaSession.setActionHandler('pause', () => {
                    ...
                    if (_useMediaSessionBridge) {
                        ...
                        if (isMuted) {
                            ...
                            // Mute
                            if (_httpAudioElement) {
                                _httpAudioElement.volume = 0.0025;
    ...
    audioSpectrumTooltip.style.fontFamily = "ui-monospace, 'Courier New', monospace";
    audioSpectrumTooltip.style.lineHeight = '1.4';
...
async function connect() {
    ...
    // Read the last user-set SNR squelch value so the server can apply it immediately
    // on connect. _lastUserSNRSliderValue is updated whenever the user moves the slider
    // and is never reset on disconnect, so it always reflects the user's intent.
    // snrSquelchThreshold() returns SNR_SQUELCH_SENTINEL (-999) when the gate is disabled.
    const minSNR = (typeof _lastUserSNRSliderValue === 'number' && typeof snrSquelchThreshold === 'function')
        ? snrSquelchThreshold(_lastUserSNRSliderValue)
        : -999;
...
function handleMessage(msg) {
    switch (msg.type) {
        ...
        case 'audio_gate_updated':
            ...
            // Sync the slider to whatever the server reports.
...
function playAudioBuffer(buffer) {
    ...
    if (audioContext._httpStreamMuted) {
        ...
        gainNode.gain.value = isMuted ? 0.0025 : currentVolume;
    }
...
function tune() {
    ...
    const _minSNRForTune = (typeof _lastUserSNRSliderValue === 'number' && typeof snrSquelchThreshold === 'function')
        ? snrSquelchThreshold(_lastUserSNRSliderValue)
        : -999;
...
// Last slider value explicitly set by the user (updated in sendSNRSquelch).
// Never reset on disconnect — used to restore the gate on reconnect.
// Initialised to SNR_SQUELCH_OFF_VAL so the gate starts disabled.
let _lastUserSNRSliderValue = SNR_SQUELCH_OFF_VAL;
...
function sendSNRSquelch() {
    ...
    _snrSquelchTimer = setTimeout(() => {
        const sl = document.getElementById('snr-squelch-slider');
        if (!sl) return;
        // Track the last user-set slider value for reconnect restore.
        _lastUserSNRSliderValue = parseFloat(sl.value);
        const t = snrSquelchThreshold(sl.value);
        if (wsManager && wsManager.ws && wsManager.ws.readyState === WebSocket.OPEN) {
            wsManager.send({ type: 'set_audio_gate', min_snr: t });
        }
...
function toggleMute() {
    ...
    if ('mediaSession' in navigator && _mediaSessionActivated) {
        if (_useMediaSessionBridge) {
            ...
            if (_httpAudioElement) {
                _httpAudioElement.volume = isMuted ? 0.0025 : 1;
            }
...
window.toggleSquelch = window.toggleAudioGate;
window.updateSquelch = updateSquelch;
window.resetSquelch = window.resetAudioGate;
 ...
 *   1. Bridge (_useMediaSessionBridge — Apple/Firefox): creates a
 *      MediaStreamDestination + hidden <audio> bridge.
 *   2. HTTP stream (_isMobileChrome — Android Chrome): creates a hidden
 *      <audio src="/audio/stream"> for the lock-screen widget.
 *   3. Metadata-only (desktop Chrome/Edge): just sets metadata and
 *      playbackState — no bridge or HTTP stream needed.
 *
...
async function setMediaSessionEnabled(enabled) {
    ...
    if (!enabled) {
        ...
        try {
            ...
            // Desktop Chrome: metadata-only — no bridge or HTTP stream needed.
            // AudioContext produces audio through destination; Chrome shows
            // media controls in the toolbar automatically.

            // All platforms: metadata + action handlers
            updateMediaSession();
            // Set playbackState immediately on bridge + desktop Chrome paths.
...
function _dockPositionWrapper() {
    ...
    // On mobile, CSS padding-bottom on the wrapper extends the background
    // down to meet the bar — no extra JS gap needed.
    ...
    if (!document.getElementById('mobile-landscape-row')) {
        ...
        if (controls) {
            ...
            if (cg1 && cg2) {
                ...
                // Right column: tab bar/panels only.
                // The band-status-bar is moved here by _syncLandscapeBandBar()
                // when in landscape, and restored to the dock wrapper in portrait.
                ...
                // Sync band bar position immediately and on orientation change
                _syncLandscapeBandBar();
                if (!window._landscapeOrientationListener) {
                    window._landscapeOrientationListener = () => _syncLandscapeBandBar();
                    window.addEventListener('resize', window._landscapeOrientationListener);
                }
            }
        }
    }
...
// browser-extension-detector.js (loaded early in index.html) sets
// window.uberSDRBridgeDetected and fires 'ubersdr:bridge-detected' on window
...
// AFTER the overlay has already been rendered (the normal case — extension waits
// up to 8 s for window.radioAPI to appear).
...
(function () {
    ...
    // Already detected before this code ran (extension injected before app.js)
    if (window.uberSDRBridgeDetected) {
        showExtensionDots();
    }

    // Normal path: extension fires the event after confirming window.radioAPI
    window.addEventListener('ubersdr:bridge-detected', function () {
        showExtensionDots();
    });
})();

...
class DigitalSpotsMap {
    ...
    setupMapTileSelector() {
        ...
        if (!select) return;
        // Checked against the table rather than trusted: a browser that saved
        // 'light' before that option was dropped would otherwise leave the
        // <select> on a value it no longer has, which renders as a blank control.
        const stored = localStorage.getItem('digspots_mapTile');

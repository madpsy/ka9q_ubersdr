// FSK/RTTY: the decoder's wire frames, its presets, and the console they fill.
//
// This decoder predates the JSON results the newer extensions send. It speaks a
// packed binary format instead — one type byte and a payload, defined in
// audio_extensions/fsk/decoder.go — so `decodeFrame` is the whole of it:
//
...
//
// All three arrive on a timer rather than on an event: text every 100 ms
// whether or not a character was decoded, the baud error every 200 ms, and the
// state only when it changes. So a panel sees a steady trickle, not a burst.
//
...
export function attachParams(config) {
    ...
    return {
        center_frequency: num(config.center_frequency, LIMITS.center_frequency, 1000),
        shift: num(config.shift, LIMITS.shift, 170),
        baud_rate: num(config.baud_rate, LIMITS.baud_rate, 45.45),
        inverted: !!config.inverted,
        framing: FRAMINGS.includes(config.framing) ? config.framing : '5N1.5',
        encoding: ENCODINGS.includes(config.encoding) ? config.encoding : 'ITA2',
    };
}
 ...
 */
export function markSpace(config) {
    const p = attachParams(config);

// AX.25 frame decoding, ported verbatim from v1's ax25decode.js.
//
// v1 in turn ported it from LinBPQ/BPQ32's Moncode.c (G8BPQ, GPL), with
// additions from the AX.25 v2.2 specification and the NET/ROM protocol
// documentation. It is pure, self-contained and already correct, so this is the
// same code with the IIFE turned into a module and nothing else changed —
// rewriting it would have meant reimplementing NET/ROM, IP, ARP, FRMR and XID
// decoding from scratch for no gain, and a partial parser would show a NODES
// broadcast as a wall of bytes.
//
// parseAX25(bytes) -> ParsedFrame | null
//
// Named parseAX25 rather than v1's bare `parse`: that is the name every
// extension panel already uses for its own frame decoder, and two of those in
// one file is a mistake waiting to be made.
//
//   from         source callsign, e.g. "GB7RDG-2"
//   to           destination callsign
//   digipeaters  digi callsigns, with * on the last one that actioned it
//   ctrl         raw control byte
//   frameClass   'I' | 'S' | 'U'
//   frameType    'ui' 'i' 'rr' 'rnr' 'rej' 'srej' 'sabm' 'sabme' 'ua' 'disc'
//                'dm' 'frmr' 'xid' 'test' 'aprs' 'netrom' 'nodes' 'ip' 'arp'
//   isCommand    true, false, or null for AX.25 v1 where it cannot be told
//   pollFinal    P/F bit
//   ns, nr       sequence numbers, for I- and S-frames
//   pid,pidName  protocol id and its name
//   info         the decoded description line
//   infoRaw      the raw info field, for a PID 0xF0 payload
//   isAPRS       a UI frame with no layer 3 — which is what APRS is
//   netrom, ip, arp, frmr   decoded payloads, when the PID says so
//
// Returns null for anything too short or malformed to be a frame. A packet
// channel delivers plenty of those and none of them is worth an exception.
 ...
 */
export function parseAX25(bytes) {

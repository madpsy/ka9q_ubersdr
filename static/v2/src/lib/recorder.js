...
export class Recorder extends Emitter {
    ...
    _pushSignal(now) {
        ...
        const np = m.noisePower == null ? -999 : m.noisePower;
        const known = bp > -900 && np > -900;

        this.signal.push({
            timestamp: new Date(now).toISOString(),
            basebandPower: bp > -900 ? bp.toFixed(2) : 'N/A',
            noisePower: np > -900 ? np.toFixed(2) : 'N/A',
            snr: known ? (bp - np).toFixed(2) : 'N/A',
        });
    }
...
`;
    ...
    // The rows are already v1-shaped strings — see _pushSignal. The middle
    // column is named for what it now holds: noise power over the passband,
    // not the density it used to be.
    _signalCsv() {
        let csv = 'UTC Time,Baseband Power (dBFS),Noise Power (dBFS),SNR (dB)\n';
        for (const s of this.signal) {
            csv += `${s.timestamp},${s.basebandPower},${s.noisePower},${s.snr}\n`;
        }
        return csv;
    }
}
